...
IMERG_dir = '/gws/nopw/j04/swift/public/requests/IMERG_for_WP2/TB3/' # Change as appropriate.
...
UM_dir_global = '/gws/nopw/j04/swift/public/requests/SWIFT_TB3/VER_FIELDS/' # Change as appropriate.
UM_dir_ta = '/gws/nopw/j04/swift/public/requests/SWIFT_TB3/VER_FIELDS/' # Change as appropriate.
...
def continuous_example():
    ...
    '''
    datelist = generate_datelist(datetime.datetime(2021,9,8,0,0,0), datetime.datetime(2021,9,10,0,0,0))
...
def read_um_data_global(forecast_day_list, varname, lon_west, lon_east, lat_south, lat_north, lead_time=0):
    ...
    for i,day in enumerate(forecast_day_list):
        if ((lead_time == 1)): # data is stored in file for day
           new_day = day-datetime.timedelta(hours=24)
           filename = UM_dir_global+'/GLOBALUM_'+varname+'_0p1deg_{:0>4}'.format(new_day.year)+'{:0>2}'.format(new_day.month)+'{:0>2}'.format(new_day.day)+'_00Z.nc'
        else:
            filename = UM_dir_global+'/GLOBALUM_'+varname+'_0p1deg_{:0>4}'.format(day.year)+'{:0>2}'.format(day.month)+'{:0>2}'.format(day.day)+'_00Z.nc'
        ...
        if lead_time == 0:
            data += [ncfile.variables[varname_dict[varname]][lat_ind1:lat_ind2+1, lon_ind1:lon_ind2+1]]
        elif lead_time == 1:
           data += [ncfile.variables[varname_dict[varname]+'_0'][lat_ind1:lat_ind2+1, lon_ind1:lon_ind2+1]]
    data = np.array(data)
...
def read_um_data_ta(forecast_day_list, varname, lon_west,lon_east, lat_south, lat_north, lead_time=0):
    ...
    missing_days=[]
    for i,day in enumerate(forecast_day_list):
        if (lead_time == 1): # data is stored in file dated 2 days ago
            new_day = day-datetime.timedelta(hours=48)
            filename = UM_dir_ta+'TAM_'+varname+'_0p1deg_{:0>4}'.format(new_day.year)+'{:0>2}'.format(new_day.month)+'{:0>2}'.format(new_day.day)+'_18Z.nc'
        else:
            new_day = day-datetime.timedelta(hours=24)
            filename =  UM_dir_ta+'TAM_'+varname+'_0p1deg_{:0>4}'.format(new_day.year)+'{:0>2}'.format(new_day.month)+'{:0>2}'.format(new_day.day)+'_18Z.nc'
...
def read_imerg_data(forecast_day_list, lon_west, lon_east, lat_south, lat_north):
    ...
    for day in forecast_day_list:
        filename = IMERG_dir+'3B-DAY-L.MS.MRG.3IMERG.{:0>4}'.format(day.year)+'{:0>2}'.format(day.month)+'{:0>2}'.format(day.day)+'-S000000-E235959.V06.nc4'
        ncfile = Dataset(filename)
